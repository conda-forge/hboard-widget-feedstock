...
Once the `conda-forge` channel has been enabled, `hboard-widget` can be installed with `conda`:
...
```
mamba install hboard-widget
```

It is possible to list all of the versions of `hboard-widget` available on your platform with `conda`:
...
```
mamba search hboard-widget --channel conda-forge
```
...
mamba repoquery search hboard-widget --channel conda-forge

# List packages depending on `hboard-widget`:
mamba repoquery whoneeds hboard-widget --channel conda-forge

# List dependencies of `hboard-widget`:
mamba repoquery depends hboard-widget --channel conda-forge
```
